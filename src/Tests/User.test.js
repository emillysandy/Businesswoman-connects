...
describe('User Class', () => {
    ...
    test('Criação da instância de User', () => {
        const user = new User('Fernanda', '56789034', 'Caruaru', 'Pernambuco');
        expect(user instanceof User).toBe(true);
    });

    test('Cadastro de usuário com dados válidos', () => {
        const user = new User();
        user.register('Sandra', '7654321', 'São Paulo', 'São Paulo');

        expect(User.data.length).toBe(1);
        expect(User.data[0].name).toBe('Sandra');
        expect(User.data[0].document).toBe('7654321');
        expect(User.data[0].city).toBe('São Paulo');
        expect(User.data[0].state).toBe('São Paulo');
    });

    test('Erro ao cadastrar usuário duplicado', () => {
        const user = new User('Fernanda', '56789034', 'Caruaru', 'Pernambuco');
        
        expect(() => user.register('Fernanda', '56789034', 'Caruaru', 'Pernambuco')).toThrow('Erro, usuário já cadastrado');
        expect(User.data.length).toBe(1);
    });

    test('Buscar usuário por documento', () => {
        const user1 = new User('Fernanda', '56789034', 'Caruaru', 'Pernambuco');
        const user2 = new User('Liz', '13579000', 'Recife', 'Pernambuco');
